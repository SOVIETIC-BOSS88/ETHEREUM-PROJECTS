...
router.post('/orders', async (req, res, next) => {
  let {currency, items, email, shipping} = await req.body;
  try {
...
router.post('/orders/:id/pay', async (req, res, next) => {
  ...
  try {
    ...
    if (source && source.status === 'chargeable') {
      let charge, status, customerId;
      try {
        //Here we will store our entire list of customers
        var customerDetails = [];
        var hasMore = true;
        var lastCustomer;
        //var index = 0;
        while(hasMore) {
          ...
          var page = await stripe.customers.list({ limit: 100, starting_after: lastCustomer});
          page.data.forEach(function(customer) {
            customerDetails.push({id: customer.id, created: customer.created, description: customer.description,
              email: customer.email, sources: customer.sources});
            lastCustomer = customer.id;
          });
          hasMore = page.has_more;
          console.log("Already got " + customerDetails.length + " customers in this account...");
        }
        console.log("Done with listing all customers");

        //Here we check if the customer exists by finding its index in the array,
        //if not it returns a -1 and we will create a customer
        var index = customerDetails.findIndex(details => details.email === order.email);

        if (index >= 0) {
          charge = await stripe.charges.create({
            amount: order.amount,
            currency: order.currency,
            customer: customerDetails[index].id,
          ...
          .then(customer => charge = stripe.charges.create({
            amount: order.amount,
            currency: order.currency,
