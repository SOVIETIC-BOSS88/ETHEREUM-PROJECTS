...
function mint() {
    ...
    xhttp.open("GET", 'https://min-api.cryptocompare.com/data/price?fsym=USD&tsyms=ETH', false);
    xhttp.setRequestHeader("Content-type", "application/json");
    xhttp.send();
    var response = JSON.parse(xhttp.responseText).ETH;
    var etherAmount = response * 2.99;
    var amountInWei = web3.toWei(''+etherAmount, 'ether');
    console.log(etherAmount);

    this.contract.mintTo(account, amountInWei, "https://www.alicetoken.com/api/1",  {from: account, value: amountInWei, gas : 250000 }, (err, result) => {
